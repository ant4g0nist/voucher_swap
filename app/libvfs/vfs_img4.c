    ...
    DERByte **img4_data, DERSize *img4_length)
{
    ...
    certChain[0].data = NULL;
    certChain[0].length = 0;
...
#if 1||DEBUG
            if ((var_A8.tag & 0xFFFFFFFF00000000) == E000000000000000) {
                printf("%s: %s: priv [ %c%c%c%c ] => %d (0x%llx)\n", __FUNCTION__, (var_90.tag == (E000000000000000 | 'OBJP')) ? "objp" : "manp", FOURCC(var_A8.tag), rv, var_108[1].tag);
            } else {
                printf("%s: %s: %llx, %c%c%c%c => %d (0x%llx)\n", __FUNCTION__, (var_90.tag == (E000000000000000 | 'OBJP')) ? "objp" : "manp", tag, FOURCC(var_A8.tag), rv, var_108[1].tag);
            }
#endif
            /* XXX (tag & 0xE...) > 0x8... => 0xA... */
...
#if 1||DEBUG
    if ((tag & 0xFFFFFFFF00000000) == E000000000000000) {
        printf("%s: %s: priv [ %c%c%c%c ]\n", __FUNCTION__, what ? "objp" : "manp", FOURCC(tag));
    } else {
        printf("%s: %s: %llx, %c%c%c%c\n", __FUNCTION__, what ? "objp" : "manp", tag, FOURCC(tag));
    }
#endif

    // XXX tag is unsigned int?
...
validate(TheImg4 *img4, unsigned type, const char *args)
{
    ...
    ctx.hardware->ECID = 0/*x00000574AA6262D0*/;
    ctx.hardware->CPRO = 1;
    ctx.hardware->CSEC = 1;
    ctx.hardware->SDOM = 1;
    ctx.hardware->SEPO = 1;
    ...
    ctx.hardware->field_30 = 0x31e1769f9bc8a511;
#elif 1
...
// there is a bug in Img4DecodeGetRestoreInfoData: if there is no nonce, it will return stack memory? XXX
